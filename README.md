...
версий = наш воркспейс), `epics.toml` (какие потоки задач существуют), `bootstrap.sh`
(восстановить набор одной командой), политики и CI-гейты. Сами репозитории — независимые клоны-соседи, здесь не трекаются.
...
| Потоки задач (эпики) | **этот зонтик** `epics.toml` | SSOT значений оси эпиков (ADR-ECO-010); грамматика — контракт `epics/v1` в KB |
...
## Изменить состав эпиков

Правка `epics.toml` — тоже **только через PR** (CODEOWNERS-аппрув), но под гейтом её пока
**нет**: автоматической проверки реестра против схемы `epics/v1` в CI не существует, валидность
держится на ревью. Сенсор — Ф1b по ADR-ECO-010, вместе с проверкой `EP-MISSING` по флоту.
До него добавление эпика с опечаткой в ключе или висячим `moved_to` не будет поймано машиной.
