...
Правка `epics.toml` — тоже **только через PR** (CODEOWNERS-аппрув) и под гейтом
`epics-registry`: он валидирует реестр против пиненой формы контракта `epics/v1` и
блокирует опечатку в ключе, висячий `moved_to`, цепочку надгробий и standing-эпик с
датой закрытия. Покрытие по флоту (`EP-MISSING`) считает ночной сенсор — см.
[`ci/epics-sensor.md`](ci/epics-sensor.md).
