...
#
# `.governance` — чекаут САМОГО ЗОНТИКА, который гейт кладёт внутрь каллера
# (`governance-gate.yml`, шаг «checkout governance scripts», `path: .governance`).
# Это дерево сканера, а не runtime-код проверяемого репо, и в рабочей копии
# потребителя его не существует вовсе.
#
# Пока его сканировали, ЛЮБОЕ упоминание токена где угодно в зонтике красило
# гейт во ВСЕХ 22 репо разом — на следующем же бампе пина. Замер 2026-09-19:
# `epics.toml:205` (прозаическая заметка «Основание: _cowork_output/...»)
# появился в зонтике после пина 51513e8; бамп на 2f80dd4 уронил impresario#53
# на файле, к самому impresario отношения не имеющем. Признак, что дыру
# латали и раньше: сам этот чекер носит `gov:allow-cowork-file` в шапке —
# маркер нужен был ровно потому, что его читали из `.governance/`.
#
# Почему это не дыра «репо спрячет код в своём `.governance/`»: сканирование
# идёт ПОСЛЕ шага checkout, который кладёт в этот путь дерево зонтика. Что бы
# каллер там ни трекал, к моменту скана по этому пути лежит зонтик, а не его
# файлы. Скрыть код таким способом нельзя — его там просто не будет.
SKIP_DIRS = {".git", ".governance", "_cowork_output", "node_modules", ".venv",
             "venv", "dist", "build", "target", "__pycache__", ".mypy_cache",
...
def verify_declarations_exist(repo: Path, data_files: list[str]) -> None:
    ...
    счёл бы исключение действующим.

    Symlink отказывается в ЛЮБОМ сегменте пути, не только в конечном: цель
    может лежать вне объявленного дерева, а `is_file()` проходит СКВОЗЬ
    symlink в родительском каталоге. Проверка только конечного компонента
    принимала `pilot/briefs -> ../tests/evidence` — объявленным считался файл,
    физически лежащий там, куда объявление не выдавалось (находка третьего
    захода приёмочного ревью, major).
    """
    for rel in data_files:
        ...
        for part in Path(rel).parts:
            ...
            if probe.is_symlink():
                raise PolicyError(
                    f"объявление data-артефакта ведёт через symlink: {rel} — "
                    f"сегмент {probe.relative_to(repo)} это symlink, его цель может "
                    f"лежать вне объявленного дерева. Поправьте "
                    f"[{DATA_ARTIFACTS_TABLE}] в caller-policy.toml зонтика."
                )
        if not probe.is_file():
            raise PolicyError(
                f"объявление data-артефакта протухло: {rel} — не обычный файл "
                f"в этом репо (удалён, переименован или каталог). "
